---
name: whatsapp-tenant-phone-assignment-task-08
description: Implementa exclusivamente a task 08 (Seed, Postman e verificação) da change whatsapp-tenant-phone-assignment. Use via /opsx-apply ou quando a task 08 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **08** da change `whatsapp-tenant-phone-assignment`.
...
`openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-08-seed-postman-e-verificacao.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/whatsapp-tenant-phone-assignment/tasks.md` as linhas do grupo 8: `- [ ]` → `- [x]` (8.1, 8.2, 8.3).
...
- `openspec/changes/whatsapp-tenant-phone-assignment/FRONT-INTEGRATION.md`
- `openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-08-seed-postman-e-verificacao.md` (apenas sua task)
