---
name: whatsapp-template-lifecycle-and-profile-task-05
description: Implementa exclusivamente a task 05 (Contratos front, Swagger e verificação) da change whatsapp-template-lifecycle-and-profile. Use via /opsx-apply ou quando a task 05 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **05** da change `whatsapp-template-lifecycle-and-profile`.
...
`openspec/changes/whatsapp-template-lifecycle-and-profile/tasks/task-05-contratos-front-swagger-e-verificacao.md` — leia **toda** a task.

Leia antes de codar: O que fazer, Critérios de aceite, Verificação (se houver).

## Workflow (/opsx-apply adaptado — uma task)

1. Confira `openspec/changes/whatsapp-template-lifecycle-and-profile/tasks.md`: verifique pré-requisitos (grupos 1–4 completos).
...
5. Marque em `openspec/changes/whatsapp-template-lifecycle-and-profile/tasks.md` as linhas 5.1–5.3: `- [ ]` → `- [x]`.
...
- `openspec/changes/whatsapp-template-lifecycle-and-profile/tasks/task-05-contratos-front-swagger-e-verificacao.md` (apenas sua task)

## Guardrails

- Mudanças mínimas; siga convenções do monorepo.
- **Proibido** usar a palavra “academia” em qualquer doc desta task.
- Não implementar UI React neste repo.
