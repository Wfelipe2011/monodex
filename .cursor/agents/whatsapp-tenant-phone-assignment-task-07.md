---
name: whatsapp-tenant-phone-assignment-task-07
description: Implementa exclusivamente a task 07 (Catálogo — sync na default) da change whatsapp-tenant-phone-assignment. Use via /opsx-apply ou quando a task 07 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **07** da change `whatsapp-tenant-phone-assignment`.
...
`openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-07-catalogo-sync-na-default.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/whatsapp-tenant-phone-assignment/tasks.md` as linhas do grupo 7: `- [ ]` → `- [x]` (7.1).
...
- `openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-07-catalogo-sync-na-default.md` (apenas sua task)
