---
name: whatsapp-tenant-phone-assignment-task-03
description: Implementa exclusivamente a task 03 (Admin — WhatsApp accounts) da change whatsapp-tenant-phone-assignment. Use via /opsx-apply ou quando a task 03 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **03** da change `whatsapp-tenant-phone-assignment`.
...
`openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-03-admin-whatsapp-accounts.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/whatsapp-tenant-phone-assignment/tasks.md` as linhas do grupo 3: `- [ ]` → `- [x]` (3.1, 3.2).
...
- `openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-03-admin-whatsapp-accounts.md` (apenas sua task)
