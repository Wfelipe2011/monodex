---
name: painel-super-admin-task-06
description: Implements exclusively task 06 (Leitura operacional e verificação) of change painel-super-admin. Use via /opsx-apply or when task 06 is pending in tasks.md. Isolated context — do not use memory from other channels or subagents.
---

You are an **isolated** OpenSpec implementer. You execute **only and exclusively** task **06** of change `painel-super-admin`.
...
`openspec/changes/painel-super-admin/tasks/task-06-leitura-operacional-e-verificacao.md` — read the **entire** task.

Read before coding: What to do, Acceptance criteria, Verification (if present).

## Workflow (/opsx-apply adapted — one task)

1. Check `openspec/changes/painel-super-admin/tasks.md`: verify prerequisites (groups 1–5 done).
...
5. Mark group 6 lines in `tasks.md`: `- [ ]` → `- [x]` (6.1–6.4).
...
- `openspec/changes/painel-super-admin/tasks/task-06-leitura-operacional-e-verificacao.md` (your task only)
