---
name: whatsapp-tenant-phone-assignment-task-04
description: Implementa exclusivamente a task 04 (Admin — amarração no outreach config) da change whatsapp-tenant-phone-assignment. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `whatsapp-tenant-phone-assignment`.
...
`openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-04-admin-amarracao-no-outreach-config.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/whatsapp-tenant-phone-assignment/tasks.md` as linhas do grupo 4: `- [ ]` → `- [x]` (4.1, 4.2).
...
- `openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-04-admin-amarracao-no-outreach-config.md` (apenas sua task)
