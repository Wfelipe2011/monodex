---
name: outreach-quota-refill-on-failure-task-04
description: Implementa exclusivamente a task 04 (List campaigns — abrir run e Graph-fail refill) da change outreach-quota-refill-on-failure. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `outreach-quota-refill-on-failure`.
...
`openspec/changes/outreach-quota-refill-on-failure/tasks/task-04-list-campaigns-abrir-run-e-graph-fail-refill.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/outreach-quota-refill-on-failure/tasks.md` as linhas 4.1–4.3: `- [ ]` → `- [x]`.
...
- `openspec/changes/outreach-quota-refill-on-failure/tasks/task-04-list-campaigns-abrir-run-e-graph-fail-refill.md` (apenas sua task)
