---
name: outreach-contact-text-task-01
description: Implementa exclusivamente a task 01 (Schema e migration) da change outreach-contact-text. Use via /opsx-apply ou quando a task 01 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **01** da change `outreach-contact-text`.
...
`openspec/changes/outreach-contact-text/tasks/task-01-schema-e-migration.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/outreach-contact-text/tasks.md` as linhas da task 1 (`1.1` e `1.2`): `- [ ]` → `- [x]`.
...
- `openspec/changes/outreach-contact-text/tasks/task-01-schema-e-migration.md` (apenas sua task)
