...
**Reason:** Contact copy and header image are slot bindings on the catalog template, not dedicated columns.
**Migration:** PUT `slotBindings.outreach` with a `literal` or `header_image` entry for the corresponding slot keys. External clients MUST stop sending `outreachContactText` and `headerImageUrl`.
