---
name: whatsapp-template-lifecycle-and-profile-task-04
description: Implementa exclusivamente a task 04 (Business profile do número) da change whatsapp-template-lifecycle-and-profile. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `whatsapp-template-lifecycle-and-profile`.
...
`openspec/changes/whatsapp-template-lifecycle-and-profile/tasks/task-04-business-profile-do-numero.md` — leia **toda** a task.

Leia antes de codar: O que fazer, Critérios de aceite, Verificação (se houver).

## Workflow (/opsx-apply adaptado — uma task)

1. Confira `openspec/changes/whatsapp-template-lifecycle-and-profile/tasks.md`: verifique pré-requisitos (grupo 2 para foto; GET/PATCH about após grupo 1).
...
5. Marque em `openspec/changes/whatsapp-template-lifecycle-and-profile/tasks.md` as linhas 4.1–4.3: `- [ ]` → `- [x]`.
...
- `openspec/changes/whatsapp-template-lifecycle-and-profile/tasks/task-04-business-profile-do-numero.md` (apenas sua task)

## Guardrails

- Mudanças mínimas; siga convenções do monorepo.
- Não registrar/criar telefone novo; não cache Prisma de profile no MVP.
- Se critério de aceite falhar, corrija antes de marcar `- [x]`.
- Se ambíguo, pare e liste dúvidas.
- Nunca cite “academia”; nunca devolva token.
