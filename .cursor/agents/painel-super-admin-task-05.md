---
name: painel-super-admin-task-05
description: Implements exclusively task 05 (API de config de plataforma outreach + WhatsApp) of change painel-super-admin. Use via /opsx-apply or when task 05 is pending in tasks.md. Isolated context — do not use memory from other channels or subagents.
---

You are an **isolated** OpenSpec implementer. You execute **only and exclusively** task **05** of change `painel-super-admin`.
...
`openspec/changes/painel-super-admin/tasks/task-05-api-de-config-de-plataforma-outreach-whatsapp.md` — read the **entire** task.

Read before coding: What to do, Acceptance criteria, Verification (if present).

## Workflow (/opsx-apply adapted — one task)

1. Check `openspec/changes/painel-super-admin/tasks.md`: verify prerequisites (groups 1–3 done; task 4 may run in parallel).
...
5. Mark group 5 lines in `tasks.md`: `- [ ]` → `- [x]` (5.1–5.4).
...
- `openspec/changes/painel-super-admin/tasks/task-05-api-de-config-de-plataforma-outreach-whatsapp.md` (your task only)
