---
name: whatsapp-tenant-phone-assignment-task-05
description: Implementa exclusivamente a task 05 (Notifly — envios por tenant) da change whatsapp-tenant-phone-assignment. Use via /opsx-apply ou quando a task 05 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **05** da change `whatsapp-tenant-phone-assignment`.
...
`openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-05-notifly-envios-por-tenant.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/whatsapp-tenant-phone-assignment/tasks.md` as linhas do grupo 5: `- [ ]` → `- [x]` (5.1, 5.2).
...
- `openspec/changes/whatsapp-tenant-phone-assignment/tasks/task-05-notifly-envios-por-tenant.md` (apenas sua task)
