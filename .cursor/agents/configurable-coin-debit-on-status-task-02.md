---
name: configurable-coin-debit-on-status-task-02
description: Implementa exclusivamente a task 02 (Notifly — serviço de débito/estorno) da change configurable-coin-debit-on-status. Use via /opsx-apply ou quando a task 02 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **02** da change `configurable-coin-debit-on-status`.
...
`openspec/changes/configurable-coin-debit-on-status/tasks/task-02-notifly-servico-de-debito-estorno.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/configurable-coin-debit-on-status/tasks.md` as linhas do grupo 2: `- [ ]` → `- [x]` (2.1–2.4).
...
- `openspec/changes/configurable-coin-debit-on-status/tasks/task-02-notifly-servico-de-debito-estorno.md` (apenas sua task)
