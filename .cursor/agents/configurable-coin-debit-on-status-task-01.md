---
name: configurable-coin-debit-on-status-task-01
description: Implementa exclusivamente a task 01 (Schema e migration) da change configurable-coin-debit-on-status. Use via /opsx-apply ou quando a task 01 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **01** da change `configurable-coin-debit-on-status`.
...
`openspec/changes/configurable-coin-debit-on-status/tasks/task-01-schema-e-migration.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/configurable-coin-debit-on-status/tasks.md` as linhas do grupo 1: `- [ ]` → `- [x]` (1.1, 1.2, 1.3).
...
- `openspec/changes/configurable-coin-debit-on-status/tasks/task-01-schema-e-migration.md` (apenas sua task)
