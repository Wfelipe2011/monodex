---
name: tenant-admin-rbac-and-send-policies-task-06
description: Implementa exclusivamente a task 06 (Pedidos de scrape pelo Admin) da change tenant-admin-rbac-and-send-policies. Use via /opsx-apply ou quando a task 06 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **06** da change `tenant-admin-rbac-and-send-policies`.
...
`openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-06-pedidos-de-scrape-pelo-admin.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-admin-rbac-and-send-policies/tasks.md` as linhas do grupo 6: `- [ ]` → `- [x]` (6.1–6.3).
...
- `openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-06-pedidos-de-scrape-pelo-admin.md` (apenas sua task)
