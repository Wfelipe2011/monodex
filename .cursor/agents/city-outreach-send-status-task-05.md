---
name: city-outreach-send-status-task-05
description: Implementa exclusivamente a task 05 (Postman, FRONT-INTEGRATION e verificação) da change city-outreach-send-status. Use via /opsx-apply ou quando a task 05 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **05** da change `city-outreach-send-status`.
...
`openspec/changes/city-outreach-send-status/tasks/task-05-postman-front-integration-e-verificacao.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/city-outreach-send-status/tasks.md` as linhas do grupo 5: `- [ ]` → `- [x]` (5.1, 5.2, 5.3).
...
- `openspec/changes/city-outreach-send-status/tasks/task-05-postman-front-integration-e-verificacao.md` (apenas sua task)
