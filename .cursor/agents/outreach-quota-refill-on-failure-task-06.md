---
name: outreach-quota-refill-on-failure-task-06
description: Implementa exclusivamente a task 06 (Verificação e handoff) da change outreach-quota-refill-on-failure. Use via /opsx-apply ou quando a task 06 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **06** da change `outreach-quota-refill-on-failure`.
...
`openspec/changes/outreach-quota-refill-on-failure/tasks/task-06-verificacao-e-handoff.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/outreach-quota-refill-on-failure/tasks.md` as linhas 6.1–6.2: `- [ ]` → `- [x]`.
...
- `openspec/changes/outreach-quota-refill-on-failure/tasks/task-06-verificacao-e-handoff.md` (apenas sua task)

## Guardrails

- Não expandir escopo; não arquivar a change.
