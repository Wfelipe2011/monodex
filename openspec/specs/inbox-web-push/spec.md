...
The system SHALL allow authenticated users to register a Web Push subscription via `PUT /admin/push-subscriptions` with body `{ endpoint, keys: { p256dh, auth } }`. Subscriptions MUST be stored linked to the authenticated `userId`. The same `endpoint` MUST upsert (update keys/user) rather than duplicate.

#### Scenario: Register subscription
- **WHEN** authenticated user PUTs valid subscription JSON
- **THEN** a `PushSubscription` row exists for that user and endpoint

#### Scenario: Delete subscription
- **WHEN** authenticated user DELETEs with `{ endpoint }` matching their subscription
...
Each push MUST include `data.url` pointing to the list lead conversation path `/admin/tenants/{tenantId}/lead-lists/{listId}/leads/{leadId}` plus `tenantId`, `listId`, `leadId`, and `messageId`.
