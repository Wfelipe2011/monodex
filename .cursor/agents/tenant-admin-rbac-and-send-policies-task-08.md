---
name: tenant-admin-rbac-and-send-policies-task-08
description: Implementa exclusivamente a task 08 (Postman, seeds e verificação) da change tenant-admin-rbac-and-send-policies. Use via /opsx-apply ou quando a task 08 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **08** da change `tenant-admin-rbac-and-send-policies`.
...
`openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-08-postman-seeds-e-verificacao.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-admin-rbac-and-send-policies/tasks.md` as linhas do grupo 8: `- [ ]` → `- [x]` (8.1–8.3).
...
- `openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-08-postman-seeds-e-verificacao.md` (apenas sua task)
