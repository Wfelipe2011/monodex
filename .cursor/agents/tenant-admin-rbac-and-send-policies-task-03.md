---
name: tenant-admin-rbac-and-send-policies-task-03
description: Implementa exclusivamente a task 03 (Prefixo /platform e split de campos Super Admin) da change tenant-admin-rbac-and-send-policies. Use via /opsx-apply ou quando a task 03 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **03** da change `tenant-admin-rbac-and-send-policies`.
...
`openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-03-prefixo-platform-e-split-de-campos-super-admin.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-admin-rbac-and-send-policies/tasks.md` as linhas do grupo 3: `- [ ]` → `- [x]` (3.1–3.5).
...
- `openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-03-prefixo-platform-e-split-de-campos-super-admin.md` (apenas sua task)
