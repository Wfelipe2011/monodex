---
name: tenant-invite-and-self-password-task-03
description: Implementa exclusivamente a task 03 (Admin — emissão, listagem e revogação) da change tenant-invite-and-self-password. Use via /opsx-apply ou quando a task 03 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **03** da change `tenant-invite-and-self-password`.
...
`openspec/changes/tenant-invite-and-self-password/tasks/task-03-admin-emissao-listagem-e-revogacao.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-invite-and-self-password/tasks.md` as linhas do grupo 3: `- [ ]` → `- [x]` (3.1, 3.2, 3.3, 3.4).
...
- `openspec/changes/tenant-invite-and-self-password/tasks/task-03-admin-emissao-listagem-e-revogacao.md` (apenas sua task)
