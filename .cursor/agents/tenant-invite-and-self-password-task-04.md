---
name: tenant-invite-and-self-password-task-04
description: Implementa exclusivamente a task 04 (Público — preview e aceite) da change tenant-invite-and-self-password. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `tenant-invite-and-self-password`.
...
`openspec/changes/tenant-invite-and-self-password/tasks/task-04-publico-preview-e-aceite.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-invite-and-self-password/tasks.md` as linhas do grupo 4: `- [ ]` → `- [x]` (4.1, 4.2, 4.3).
...
- `openspec/changes/tenant-invite-and-self-password/tasks/task-04-publico-preview-e-aceite.md` (apenas sua task)
