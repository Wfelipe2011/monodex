---
name: tenant-api-keys-on-demand-send-task-06
description: Implementa exclusivamente a task 06 (Notifly — billing on-demand, webhook e reserva unificada) da change tenant-api-keys-on-demand-send. Use via /opsx-apply ou quando a task 06 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **06** da change `tenant-api-keys-on-demand-send`.
...
`openspec/changes/tenant-api-keys-on-demand-send/tasks/task-06-notifly-billing-on-demand-webhook-e-reserva-unificada.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-api-keys-on-demand-send/tasks.md` as linhas do grupo 6: `- [ ]` → `- [x]` (6.1–6.4).
...
- `openspec/changes/tenant-api-keys-on-demand-send/tasks/task-06-notifly-billing-on-demand-webhook-e-reserva-unificada.md` (apenas sua task)
