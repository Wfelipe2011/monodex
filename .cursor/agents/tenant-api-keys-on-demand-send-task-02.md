---
name: tenant-api-keys-on-demand-send-task-02
description: Implementa exclusivamente a task 02 (Shared — hash de chave e AuthGuard dual-mode) da change tenant-api-keys-on-demand-send. Use via /opsx-apply ou quando a task 02 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **02** da change `tenant-api-keys-on-demand-send`.
...
`openspec/changes/tenant-api-keys-on-demand-send/tasks/task-02-shared-hash-de-chave-e-authguard-dual-mode.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-api-keys-on-demand-send/tasks.md` as linhas do grupo 2: `- [ ]` → `- [x]` (2.1–2.4).
...
- `openspec/changes/tenant-api-keys-on-demand-send/tasks/task-02-shared-hash-de-chave-e-authguard-dual-mode.md` (apenas sua task)
