---
name: configurable-coin-debit-on-status-task-04
description: Implementa exclusivamente a task 04 (Admin — config e contratos) da change configurable-coin-debit-on-status. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `configurable-coin-debit-on-status`.
...
`openspec/changes/configurable-coin-debit-on-status/tasks/task-04-admin-config-e-contratos.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/configurable-coin-debit-on-status/tasks.md` as linhas do grupo 4: `- [ ]` → `- [x]` (4.1–4.3).
...
- `openspec/changes/configurable-coin-debit-on-status/tasks/task-04-admin-config-e-contratos.md` (apenas sua task)
