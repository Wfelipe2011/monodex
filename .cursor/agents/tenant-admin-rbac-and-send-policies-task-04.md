---
name: tenant-admin-rbac-and-send-policies-task-04
description: Implementa exclusivamente a task 04 (Superfície /tenant operacional) da change tenant-admin-rbac-and-send-policies. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `tenant-admin-rbac-and-send-policies`.
...
`openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-04-superficie-tenant-operacional.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-admin-rbac-and-send-policies/tasks.md` as linhas do grupo 4: `- [ ]` → `- [x]` (4.1–4.6).
...
- `openspec/changes/tenant-admin-rbac-and-send-policies/tasks/task-04-superficie-tenant-operacional.md` (apenas sua task)
