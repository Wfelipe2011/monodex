---
name: outreach-quota-refill-on-failure-task-05
description: Implementa exclusivamente a task 05 (Webhook failed + billing hooks) da change outreach-quota-refill-on-failure. Use via /opsx-apply ou quando a task 05 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **05** da change `outreach-quota-refill-on-failure`.
...
`openspec/changes/outreach-quota-refill-on-failure/tasks/task-05-webhook-failed-billing-hooks.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/outreach-quota-refill-on-failure/tasks.md` as linhas 5.1–5.3: `- [ ]` → `- [x]`.
...
- `openspec/changes/outreach-quota-refill-on-failure/tasks/task-05-webhook-failed-billing-hooks.md` (apenas sua task)
