---
name: tenant-api-keys-on-demand-send-task-07
description: Implementa exclusivamente a task 07 (Agendas — API e worker horário) da change tenant-api-keys-on-demand-send. Use via /opsx-apply ou quando a task 07 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **07** da change `tenant-api-keys-on-demand-send`.
...
`openspec/changes/tenant-api-keys-on-demand-send/tasks/task-07-agendas-api-e-worker-horario.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-api-keys-on-demand-send/tasks.md` as linhas do grupo 7: `- [ ]` → `- [x]` (7.1–7.4).
...
- `openspec/changes/tenant-api-keys-on-demand-send/tasks/task-07-agendas-api-e-worker-horario.md` (apenas sua task)
