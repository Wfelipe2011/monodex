---
name: tenant-api-keys-on-demand-send-task-05
description: Implementa exclusivamente a task 05 (Gym-ctrl — send on-demand e dual-auth nas rotas existentes) da change tenant-api-keys-on-demand-send. Use via /opsx-apply ou quando a task 05 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **05** da change `tenant-api-keys-on-demand-send`.
...
`openspec/changes/tenant-api-keys-on-demand-send/tasks/task-05-gym-ctrl-send-on-demand-e-dual-auth-nas-rotas-existentes.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-api-keys-on-demand-send/tasks.md` as linhas do grupo 5: `- [ ]` → `- [x]` (5.1–5.4).
...
- `openspec/changes/tenant-api-keys-on-demand-send/tasks/task-05-gym-ctrl-send-on-demand-e-dual-auth-nas-rotas-existentes.md` (apenas sua task)
