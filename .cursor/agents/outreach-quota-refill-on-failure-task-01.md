---
name: outreach-quota-refill-on-failure-task-01
description: Implementa exclusivamente a task 01 (Schema e migration) da change outreach-quota-refill-on-failure. Use via /opsx-apply ou quando a task 01 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **01** da change `outreach-quota-refill-on-failure`.
...
`openspec/changes/outreach-quota-refill-on-failure/tasks/task-01-schema-e-migration.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/outreach-quota-refill-on-failure/tasks.md` as linhas 1.1–1.3: `- [ ]` → `- [x]`.
...
- `openspec/changes/outreach-quota-refill-on-failure/tasks/task-01-schema-e-migration.md` (apenas sua task)

## Guardrails

- Mudanças mínimas; siga convenções em `prisma/`.
