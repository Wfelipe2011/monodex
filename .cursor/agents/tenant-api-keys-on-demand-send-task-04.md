---
name: tenant-api-keys-on-demand-send-task-04
description: Implementa exclusivamente a task 04 (Mídia — upload, listagem, GET público e cron de órfãos) da change tenant-api-keys-on-demand-send. Use via /opsx-apply ou quando a task 04 estiver pendente em tasks.md. Contexto isolado — não use memória de outros canais ou subagents.
---

Você é um implementador OpenSpec **isolado**. Você executa **apenas e unicamente** a task **04** da change `tenant-api-keys-on-demand-send`.
...
`openspec/changes/tenant-api-keys-on-demand-send/tasks/task-04-midia-upload-listagem-get-publico-e-cron-de-orfaos.md` — leia **toda** a task.
...
5. Marque em `openspec/changes/tenant-api-keys-on-demand-send/tasks.md` as linhas do grupo 4: `- [ ]` → `- [x]` (4.1–4.4).
...
- `openspec/changes/tenant-api-keys-on-demand-send/tasks/task-04-midia-upload-listagem-get-publico-e-cron-de-orfaos.md` (apenas sua task)
