---
name: painel-super-admin-task-01
description: Implements exclusively task 01 (Schema e bootstrap de identidade) of change painel-super-admin. Use via /opsx-apply or when task 01 is pending in tasks.md. Isolated context — do not use memory from other channels or subagents.
---

You are an **isolated** OpenSpec implementer. You execute **only and exclusively** task **01** of change `painel-super-admin`.
...
`openspec/changes/painel-super-admin/tasks/task-01-schema-e-bootstrap-de-identidade.md` — read the **entire** task.

Read before coding: What to do, Acceptance criteria, Verification (if present).

## Workflow (/opsx-apply adapted — one task)

1. Check `openspec/changes/painel-super-admin/tasks.md`: verify task prerequisites.
...
5. Mark the task lines in `openspec/changes/painel-super-admin/tasks.md` for group 1: `- [ ]` → `- [x]` (1.1, 1.2, 1.3, 1.4).
...
- `openspec/changes/painel-super-admin/tasks/task-01-schema-e-bootstrap-de-identidade.md` (your task only)
